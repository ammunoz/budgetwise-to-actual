...
import { signAmount, toISODate, shiftISODateMonths } from './normalize.js';
...
async function importAccounts(accounts) {
  ...
  for (const a of accounts) {
    const initialBalance = signAmount(a.current_bal ?? '0', a.type === 'credit' ? 'outflow' : 'inflow');
  ...
  logger.info(`  ${accountIdToActual.size} accounts`);
...
function buildActualTx(bwTx, accountIdToActual, categoryIdToActual, payeeNameToActual, incomeCategoryId, transferPayeeByAccount, outflowDestAccountById) {
  ...
  return {
    ...
    date: shiftISODateMonths(toISODate(bwTx.date), bwTx.ltb_next ? 1 : 0),
...
function buildSubtransactions(parentBw, parentActualId, categoryIdToActual, payeeNameToActual, incomeCategoryId, transferPayeeByAccount, outflowDestAccountById) {
  return parentBw.child_transactions.map((c) => ({
    date: shiftISODateMonths(toISODate(c.date ?? parentBw.date), c.ltb_next ? 1 : 0),
...
async function importTransactions({
  ...
  let initialBalanceSkipped = 0;
  let ltbRouted = 0;
  let outflowHalves = 0;
  for (const tx of transactions) {
    if (skipIds.has(tx.id)) continue;
    if (tx.payee === 'Initial Balance') { initialBalanceSkipped++; continue; }
  ...
  if (initialBalanceSkipped > 0) {
    logger.info(`  skipped ${initialBalanceSkipped} "Initial Balance" txs (covered by account.initialBalance)`);
  }
  if (ltbRouted > 0) {
  ...
  for (const [bwAccountId, txs] of byAccount) {
    ...
    for (const tx of txs) {
      ...
      if (isSplitParent) {
        ...
        parent.id = parentId;
        const subs = buildSubtransactions(tx, parentId, categoryIdToActual, payeeNameToActual, incomeCategoryId, transferPayeeByAccount, outflowDestAccountById);
  ...
  return { skipIds, importedCount, splitCount };
}
