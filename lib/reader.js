...
export async function loadCapture(capturedDir) {
  ...
  // Apply budget-row consolidation. Budgetwise's API returns multiple rows
  // per (timeframe, category_id) — one per save — with the same `spent` but
  // potentially different `budgeted`. Budgetwise's OWN per-month math (see
  // ltbBreakdown.budgetedForMonth) treats these rows as additive: the month's
  // total is the sum of all rows for that (month, category). We mirror this:
  // collapse to one row per (timeframe, category_id) [or (timeframe, cc_account_id)
  // for CC], with `budgeted` set to the SUM of all contributing rows.
  //
  // Without this, setBudgetAmount is called per row and Actual's
  // last-write-wins behavior overwrites the real value with whatever the
  // last row happens to be (often a trailing $0 echo, silently zeroing out
  // budgets).
...
// =========================================================================
// TimeframeCategory consolidation (SUM rule)
// =========================================================================
...
// fresh id and a `budgeted` value that may differ from the original.
//
// We collapse rows per `(timeframe, category_id)` [or `(timeframe,
// cc_account_id)` for CC rows], summing the `budgeted` value across all
// contributing rows. The metadata (id, spent, etc.) on the kept entry is
// taken from the first-seen row of the group.
//
// Why SUM (and not first-wins or last-wins): Budgetwise's own per-month LTB
// math (see `ltbBreakdown.budgetedForMonth`) sums all rows for that
// (month, category). Verified empirically via an oracle script: for 81/81
// months across the captured dataset, Σ raw `budgeted` per month equals
// `budgetedForMonth` exactly to the penny. First-wins / last-wins do not
// satisfy this identity (e.g., for a Category X month with two non-zero rows: $230 + $150 → sum=$380
// matches ltb; first-wins=$230 and last-wins=$150 do not).
//
// Why CC rows are id-deduped first: the BW API emits the same CC row twice
// in the response envelope with different `spent` values (likely two-pass
// update). The `budgeted` value is identical and should count once. The
// id-dedup is idempotent for non-CC rows (every id is unique within a
// non-CC group) but matters for CC.
//
...
function dedupTimeframeCategories(payload) {
  ...
  // Track per-group state separately from the output entries.
  const sumByKey = new Map();       // key → running sum of budgeted
  const entryByKey = new Map();      // key → first-seen entry (cloned, mutated)
  const seenIdsByKey = new Map();    // for CC: key → Set of seen ids
  let summedRows = 0;                // count of additional rows added to a kept row
  ...
  for (const entry of entries) {
    ...
    if (!key) {
      ...
      entryByKey.set(synth, entry);
      continue;
    }

    // CC rows: dedup by id within the group before summing (the BW API may
    // emit the same row twice with different `spent` values; the `budgeted`
    // value is what matters and should count once). For non-CC rows, every
    // row is a unique save event and contributes to the sum.
    if (entry.is_cc) {
      let seen = seenIdsByKey.get(key);
      if (!seen) { seen = new Set(); seenIdsByKey.set(key, seen); }
      if (seen.has(entry.id)) continue;
      seen.add(entry.id);
    }

    const budgeted = parseFloat(entry.budgeted || '0');
    const current = sumByKey.get(key);
    if (current === undefined) {
      // First occurrence for this key: clone the entry and seed the sum.
      sumByKey.set(key, budgeted);
      entryByKey.set(key, { ...entry, budgeted: budgeted.toFixed(2) });
    } else {
      // Subsequent occurrence: add this row's budgeted to the running sum
      // and update the kept entry's `budgeted` field.
      const next = current + budgeted;
      sumByKey.set(key, next);
      entryByKey.get(key).budgeted = next.toFixed(2);
      summedRows++;
    }
  }

  if (summedRows > 0) {
    logger.info(`  budget dedup: summed ${summedRows} additional row(s) into the kept row(s)`);
  }

  return { data: Array.from(entryByKey.values()) };
}
...
export function findFuzzyCandidates(payees) {
  const findings = [];
