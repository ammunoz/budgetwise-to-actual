...
// Compute a tx's effective YYYY-MM-DD date for budget attribution purposes.
// Honors `ltb_next` by shifting forward one month, with day-clamping.
// `rawDate` may be either an ISO datetime string or a YYYY-MM-DD string.
export const effectiveDate = (rawDate, ltbNext) =>
  shiftISODateMonths(toISODate(rawDate), ltbNext ? 1 : 0);
